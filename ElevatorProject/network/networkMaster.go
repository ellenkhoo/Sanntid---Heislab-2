...
func AnnounceMaster(ctx context.Context, localIP string, port string) {
	...
	for {
		select {
		case <- ctx.Done():
			fmt.Printf("Shutting down AnnounceMaster")
			return
		default:
			msg := "I am Master"
			conn.Write([]byte(msg))
...
func (ac *ActiveConnections) ListenAndAcceptConnections(ctx context.Context, port string, networkChannels *sharedConsts.NetworkChannels) {
	...
	for {
		select {
		case <- ctx.Done():
			fmt.Printf("Shutting down ListenAndAcceptConnections")
			return
		default:
			...
			go ReceiveMessage(ctx, networkChannels.ReceiveChan, hostConn)
			ac.AddHostConnection(hostConn, networkChannels.SendChan) //Tidliger go routine, kanskje gjør det igjen om ting går feil
		}
	}	
}
...
func (ac *ActiveConnections) MasterSendMessages(ctx context.Context, client *ClientConnectionInfo) {
	...
	for msg := range client.Channels.SendChan {
		select {
		case <- ctx.Done():
			fmt.Printf("Shutting down MasterSendMessages")
			return
		default:
			switch msg.Target {
...
func (masterData *MasterData) HandleReceivedMessagesToMaster(ctx context.Context, ac *ActiveConnections, msg sharedConsts.Message, client *ClientConnectionInfo, ackTracker *AcknowledgeTracker) {

	for {
		select {
		case <- ctx.Done():
			fmt.Printf("Shutting down HandleReceiveMessagesToMaster")
			return
		default:
